...
skill recommends. Three of them came back empty, and that emptiness is
worth writing down, because the alternative is quoting one of the
invented figures in `numbers-that-are-not-real.md`.

If you find one of these, it is genuinely new information.

## Nobody has published an iOS widget number

Not adoption share, not a retention delta, not anything measured through
`WidgetCenter.getCurrentConfigurations`. Checked: Charlie Chapman, Andy
Allen, David Smith, Curtis Herbert, which is close to the complete set of
indie developers who publish numbers at all.

The sharpest version of the gap: **Chapman published a full public
analytics post for Dark Noise** with monthly actives, daily actives, the
Mac share of sessions and OS version spread, and **said nothing about
widgets**, despite having shipped them since iOS 14.

The Android case study stands alone, and it is correlational.

**So the honest position is:** widgets are worth building for reasons of
mechanism, not for a measured effect that nobody has demonstrated on this
platform. Anyone quoting you an iOS widget retention percentage is
...
This is **structurally unmeasured, not merely unpublished**: App Store
Connect Analytics has no share-extension-source dimension, so nobody can
see it even if they want to. Argue the loop from mechanism. Do not attach
a percentage to it.

## Nobody has published a Custom Product Page result of their own

Every CPP conversion figure in circulation is vendor-published, including
the ones in this skill. No named developer has written "I shipped five
custom product pages and my conversion went from X to Y."
...
- Any firsthand account of the **Featuring Nominations form** producing a
  feature. Apple's claim that editors read every submission remains the
